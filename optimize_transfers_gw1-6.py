...
OUT=ROOT/'best_squad_with_transfers_gw1-6.json'
BUDGET=100.0
GWS=list(range(1,7))
raw=json.loads(SOURCE.read_text(encoding='utf-8'))
players=[x for x in raw['players'] if not x.get('positionMismatch')]
seen=set(); ps=[]
for x in players:
...
for g,gw in enumerate(GWS):
    for i in range(n):
        obj[ix(i,g)]=-pts(i,gw); obj[ic(i,g)]=-pts(i,gw)
...
result={'source':str(SOURCE),'assumptions':{'budget':'£100m per squad snapshot; price changes ignored because no future price data','squad':'2 GK, 5 DEF, 5 MID, 3 FWD','maxPerClub':3,'freeTransfers':'1 per GW; unused transfers carried; maximum bank 5','transferHits':'none; never exceeds available free transfers','captain':'optimised each GW; double points included','chips':'none'},'objectivePredictedPoints':round(float(-res.fun),4),'squadCostByGW':[],'transfers':[],'gameweeks':[]}
for g,gw in enumerate(GWS):
