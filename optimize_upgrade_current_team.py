...
OUT=ROOT/'best_upgrade_current_team_gw1-6.json'
...
for g in range(6):
    ...
    if g==0:
        # Only defenders may be changed before GW1; all current non-defenders stay.
        for i,x in enumerate(players):
            if x.get('position') != 'DEF':
                add([(isq(i,g),1)],1 if x.get('requestedName') in CURRENT else 0,1 if x.get('requestedName') in CURRENT else 0)
        add([(isq(i,g),1) for i,x in enumerate(players) if x.get('position')=='DEF' and x.get('requestedName') not in CURRENT],-np.inf,3)
        add([(isq(i,g),1) for i,x in enumerate(players) if x.get('requestedName')=='Thiaw'],0,0)
...
for g in range(1,6):
    ...
    # Only defenders may be transferred in later gameweeks.
    for i,x in enumerate(players):
        if x.get('position') != 'DEF':
            add([(iin(i,g-1),1)],0,0)
            add([(iout(i,g-1),1)],0,0)
    # bank cannot exceed 5, and can use no more than available bank + new free transfer
    add([(iin(i,g-1),1) for i in range(n)]+[(ib(g-1),-1)],-np.inf,1)
