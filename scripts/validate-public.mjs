...
const allowed = new Set(['app.js', 'index.html', 'predictions.html', 'predictions.js', 'styles.css', 'assets/fixtures.json', 'assets/players.json']);
const forbidden = [
...
const [privateFixtures, privatePlayers, publicFixtures, publicPlayers] = await Promise.all([
...
for (const player of publicPlayers.players) {
  ...
  for (const fixture of player.fixtures) {
    ...
    if (!privateFixture || Number(fixture.points).toFixed(1) !== Number(privateFixture.predictions.points || 0).toFixed(1)) fail(`Displayed points mismatch for ${player.fullName}, GW ${fixture.gameweek}.`);
  }
}
