...
OUT = ROOT / 'best_squad_gw1-6.json'
BUDGET = 100.0
GWS = list(range(1, 7))

p = json.loads(SOURCE.read_text(encoding='utf-8'))
players = [x for x in p['players'] if not x.get('positionMismatch')]
...
result={'source':str(SOURCE),'assumptions':{'budget':BUDGET,'squad':'2 GK, 5 DEF, 5 MID, 3 FWD','maxPerClub':3,'noTransfers':'one fixed squad for GW1-GW6','captain':'optimised independently each GW; double points included','chips':'none'},'objectivePredictedPoints':round(float(-res.fun),4),'squad':[], 'gameweeks':[]}
for x in squad:
