...
test('workflow gates artifact upload and deployment on a successful refresh', async () => {
  ...
  assert.match(workflow, /Deploy sanitized bundle to Cloudflare Pages[\s\S]*if: steps\.refresh\.outputs\.has_upcoming == 'true'[\s\S]*uses: cloudflare\/wrangler-action@v3/);
